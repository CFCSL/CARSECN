...
data = {'punt': list(range(1,8)), 'X': [0,2,2,0,1,0.05,1.95],'Y':[0,0,2,2,1,0.05,0.05]}
...
# 
# if st.button('Add rows'):
#     add_rows=pd.DataFrame('', index=range(3), columns=df_puntos.columns)
#     df_puntos=pd.concat([df_puntos,add_rows], axis=0)
#     df_puntos=st.experimental_data_editor(df_puntos)
# 
#     
# if st.button('Clear table'):
#     df_puntos= pd.DataFrame('', index=range(7), columns= df_puntos.columns)
#     df_puntos=st.experimental_data_editor(df_puntos)
#     
# =============================================================================
    
edited_df_puntos=st.experimental_data_editor(df_puntos).to_dict('records')





DB['puntos'] = edited_df_puntos
...
df_hc = pd.DataFrame({'Punto_Central': [5], 'Radio': [0.05]})

st.write('Template table **hc**: ')
st.write(df_hc)


def add_rows(df):
    df_add = pd.DataFrame('', index=[0], columns=df.columns)
    df = pd.concat([df, df_add], axis=0)
    return df



DB['hc'] =df_hc.to_dict('records')
...
df_Caracteristicas_single= st.experimental_data_editor(df_Caracteristicas_single)
single = df_Caracteristicas_single.to_dict('records')
...
df_Caracteristicas=st.experimental_data_editor(df_Caracteristicas)
multi  = df_Caracteristicas.to_dict('records')
...
df_Caracteristicas_single_pret=st.experimental_data_editor(df_Caracteristicas_single_pret)
single_pret = df_Caracteristicas_single_pret.to_dict('records')
...
df_Caracteristicas_pret=st.experimental_data_editor(df_Caracteristicas_pret)
#response = AgGrid(df_Caracteristicas_pret, editable=True, fit_columns_on_grid_load=True)
multi_pret  = df_Caracteristicas_pret.to_dict('records')
...
with open(file_txt, "rb") as fp:
	btn = st.download_button(label="Download",data=fp,file_name="Carsec_AutoGenerated.txt",mime="application/txt")
